...
public class BbsPager {
	...
	//생성자
	//BbsPager (레코드 갯수, 현재 페이지 번호)
	public BbsPager(int count, int curPage) {
	...
	public void setBlockRange() {
		...
		if (nextPage >= totPage) nextPage = totPage; //마지막 페이지가 범위를 넘지 않도록 처리
	}
	public void setPageRange() {
		//WHERE rn BETWEEN #{start} AND #{end}
		pageBegin = (curPage-1)*PAGE_SCALE+1; //시작번호 = (현재 페이지-1)*페이지 당 게시물 수 +1
		pageEnd = pageBegin+PAGE_SCALE-1; //끝번호 = 시작번호+페이지 당 게시물 수 -1
	}
	...
	public void setTotPage(int count) {
		totPage = (int)Math.ceil(count*1.0 / PAGE_SCALE); //Math.ceil(실수)올림 처리
	}
